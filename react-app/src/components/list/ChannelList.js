...
export default function ChannelList() {
    ...
    const shortenName = (input) => {
        if (input.length > 14) {
            const firstHalf = input.slice(0, 14);
            return firstHalf + "...";
        } else return input;
    };
    ...
    return (
        <>
            <div className={styles.channelListContainer}>
                ...
                <div
                    className={styles.channelListNavBar}
                    onClick={() => setShowMenu(true)}
                >
                    <div>{shortenName(serverName)}</div>
                ...
                <div>
                    {channelsArr.map((channel) => {
                        return (
                            <div
                                className={styles.eachChannelContainer}
                                key={channel.id}
                                onClick={() => {
                            ...
                            >
                                <div className={styles.hashtagNameContainer}>
                                    ...
                                    <div>{shortenName(channel.name)}</div>
                                </div>
                                {user.id === server.owner_id && (
                                    <div
                                        className={styles.gearIconContainer}
                                        onClick={() => {
